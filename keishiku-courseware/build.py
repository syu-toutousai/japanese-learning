...
SENTS = {
    ...
    # ---- きり ----
    "kiri1": {"jp": "息子は今朝、出かけたきり、まだ帰ってこない。", "cn": "儿子今早出门后，到现在还没回来。"},
    "kiri2": {"jp": "前の彼女とは、別れたきり一度も会っていません。", "cn": "和前女友分手后，就再没见过一面。"},
    "kiri3": {"jp": "今度、二人きりで食事しない？", "cn": "下次，就我们两个人单独吃个饭吧？"},
    "kiri4": {"jp": "残り時間はあと5分きりしかない。", "cn": "剩下的时间只有最后5分钟了。"},
    # ---- よう ----
...
GROUPS = [
    ...
    {"id": "time",  "name": "时间定位器",   "desc": "在时间轴上钉针、开窗、设按钮",
     "color": "#188a52", "nouns": ["tokoro", "uchi", "tabi", "kiri"]},
...
NOUNS = {
    ...
    "kiri": {
        "group": "time", "name": "きり", "jlpt": "N3", "emoji": "🎬",
        "img": "一刀切下的断点：那一刻之后、再也没有下一步——既是「唯一的一次/只剩」，也是「从此冻结」。",
        "attach": ["Ｖた＋きり", "名＋きり（で）"],
        "patterns": [
            ["Ｖた＋きり", "做了…之后（就）再没…（后续多为否定）"],
            ["〜きりだ／〜きり～ない", "从那以后一直…（状态冻结）"],
            ["名＋きり", "只有…、仅剩…（数量有限）"],
            ["名＋きり＋で（はじめて）", "就着这唯一一个…（便…）"],
        ],
        "sents": ["kiri1", "kiri2", "kiri3", "kiri4"],
        "note": "きり是「準助詞」：前接 Vた 表示时间断点后状态不再变化（常与ない连用，带牵挂/不满）；前接名词表示「只、仅」的数量限制。",
    },
    "you": {
...
CONTRASTS = [
    ...
    {
        "title": "きり vs だけ", "sub": "断点边界 vs 范围限定",
        "rule": "Ｖたきり＝从那之后没再…（时间断点，常否定）　｜　だけ＝只是范围里这么多（中性限定）",
        "rows": [
            {"mark": "○", "aid": "kiri2", "jp": "彼女とは別れた<b>きり</b>、会っていない。", "cn": "分手的瞬间之后、再无下文 → Vたきり"},
            {"mark": "○", "aid": "kiri3", "jp": "二人<b>きり</b>で話そう。", "cn": "就你我两个人（关起门来）→ 名+きり"},
            {"mark": "△", "aid": None, "jp": "君<b>だけ</b>に教える。", "cn": "只教你一个人（中性范围）→ だけ"},
        ],
    },
]
...
QA = [
    ...
    {"type": "choice", "q": "「出かけたきり、まだ帰ってこない。」中きり强调：", "opts": ["出门之后、直到现在再没回来（时间断点）", "出门时动作的伴随", "出门这个动作反复发生"], "ans": 0,
     "exp": "Vた＋きり＝在那之后状态不变、再无下文，常配带牵挂的否定。"},
    {"type": "choice", "q": "「二人きりで話そう」中きり表示：", "opts": ["只有我们两个人（数量限制）", "两个人一起做一件事", "两个人在同一时间点"], "ans": 0,
     "exp": "名＋きり＝只、仅，限定数量到「唯一一批」。"},
    {"type": "choice", "q": "「別れたきり、一度も会っていません。」的意思是？", "opts": ["分手后就再没见过一面", "分手后发现还能常见面", "分手没多久又复合了"], "ans": 0,
     "exp": "Vた＋きり＋（否定）＝那一别之后、从此再无相见。"},
]
...
QB = [
    ...
    {"type": "choice", "q": "学生時代、よく川で泳いだ（　　）だ。", "opts": ["もの", "こと", "わけ"], "ans": 0,
     "exp": "「〜たものだ」＝怀旧滤镜下的当年日常。"},
    {"type": "choice", "q": "彼女とは別れた（　　）、一度も会っていない。", "opts": ["きり", "だけ", "ところ"], "ans": 0,
     "exp": "分手那一刻的断点之后、再无下文 → Vた＋きり。"},
    {"type": "choice", "q": "君（　　）にこの秘密を教えるよ。", "opts": ["だけ", "きり", "たび"], "ans": 0,
     "exp": "中性范围「只告诉你」→ だけ。きり偏「唯一的那一次/断点」。"},
]
...
QD = [
    ...
    {"type": "choice", "q": "祖母は去年亡くなった（　　）、ずっと散歩していません。", "opts": ["きり", "ながら", "まま"], "ans": 0,
     "exp": "自祖母去世那一刻之后、再无通常的散步 → Vた＋きり。"},
]
...
function renderStudy(){
  ...
  <div class="imgbox">📦 你已经学过第15位成员：<span class="linky" onclick="alert('打开同系列的「次第」课件复习：\\nscratch/shidai-courseware/index.html')">次第</span>
  （ます形＋次第＝一…就；名词＋次第だ＝全凭…）——它也是形式名词家族的一员。番外彩蛋，不算十四杰之内。</div></div>`;
  $("#main").innerHTML=h;
}
