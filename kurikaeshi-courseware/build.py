...
诚实化立场：这不是课本正规条目，而是口语/新闻/小说/番剧里满地跑的「N0 野生活法」。
能用/不能用的界限在「重ねるだけ積み上がるか」——移动・無意図・一度きりは重ねられない。
...
def build_questions(groups, items, audio):
    ...
    def key_of(it):
        # 挖空用核心：去掉词尾一变（待った→待っ / 待って→待っ / 待つ→待っ / 泣いた→泣い）
        w = it["word"]
    ...
    for it in items:
        ...
        if ex:
            ...
            add("fill", f"{iid}:fill", type="choice",
                q=f'運用填空：{blanked}<br>'
                  f'<span class="hint">（　）に入るのは、同じ動詞を〈に〉で重ねたあの型。</span>',
...
header{background:linear-gradient(135deg,#c2410c,#f59e0b);color:#fff;padding:26px 20px 20px}
...
/* intro */
.intro{background:linear-gradient(135deg,#fff4ec,#fffaf2)}
.intro h2{font-size:17px;margin-bottom:8px;color:#c2410c}
...
function renderList(){
  ...
  <p>同一个动词说两遍：<b>待ちに待った</b>／<b>考えに考えた</b>／<b>泣きに泣いた</b>。中间的<code class="inline">に</code>
  不是时间也不是目的地，而是<code class="inline">同じ語を重ねる軸</code>——「何度も何度も」说不出的执念与程度，
  靠一个〈に〉+动词重复就演完。课本把它当<b>传说中的 N0</b>（超纲却日常到不行），其实口语、新闻、小说、番剧遍地都是。
  ...
  h+=GROUPS.map(g=>{
    ...
    return `<div class="grp"><div class="grp-h" style="background:${g.color}"><h3>${g.name}</h3><span>${members.length} 型</span></div>
...
function renderDetail(){
  ...
  GROUPS.forEach(g=>{
    ...
    members.forEach(n=>{
      const iid=n.id;
      const tagChip=n.tag?`<span class="chip pfx" title="型・用法">${n.tag}</span>`:"";
      const formChip=`<span class="chip sfx" title="公式">V連用形 ＋ に ＋ 同動詞</span>`;
      h+=`<div class="card noun" id="n-${iid}" style="--g:${g.color};--g-bg:${g.color}14">
        <h2>${n.emoji||"🔁"} ${n.word}<span class="jl">${n.level}</span></h2>
        <div class="meta">${tagChip}${formChip}
          <code>${n.read}</code>
          ${AUDIO[iid]?`<button class="btn mini-btn" title="听读音" onclick="play('${iid}',this)">▶</button>`:""}
        </div>
        <div class="meanbox">📌 <b>働き</b>　${n.meaning}${n.pattern?`<br>🧩 <b>型</b>　${n.pattern}`:""}</div>
...
function wrongBook(){return lsGet("kurikaeshi-wrong",{})}
function addWrong(ref){const w=wrongBook();w[ref]=1;lsSet("kurikaeshi-wrong",w);}
function delWrong(ref){const w=wrongBook();delete w[ref];lsSet("kurikaeshi-wrong",w);}
...
function renderQuizTab(){
  if(!mode){
    ...
    $("#main").innerHTML=`<div class="card" style="text-align:center;padding:28px 16px">
      ...
      <div class="hint" style="margin-bottom:18px">题目由 kurikaeshi.json 自动生成 · 条目越多题库越大 · 全部随机打乱</div>
      ...
      ${wc?`<button class="opt" style="max-width:220px;margin:14px auto 0;font-size:13px;padding:8px" onclick="if(confirm('清空错题本？')){localStorage.removeItem('kurikaeshi-wrong');renderQuizTab();}">🗑️ 清空错题本</button>`:""}
    </div>`;
    return;
  }
  startQuiz(mode);
...
function finish(){
  ...
  const msg=pct===100?"🏆 完璧！重ねて重ねて、意味の極みまで！":pct>=70?"👍 かなりいい！错题趁热打铁":"📖 詳細タブで復習してから再挑戦";
...
def main():
    ...
    tags = (f"<span>{len(items)} 型</span><span>{n_group} 大分類</span>"
            f"<span>{len(audio)} 音声{' + ' + str(n_nade) + ' 原声' if n_nade else ''}</span>"
            f"<span>kurikaeshi.json 随手加</span>")
